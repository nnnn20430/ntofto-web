function getPubkeySync() {
  $.ajax({
      url: './compute.php',
      type: 'post',
      data: 'keyRequest=true',
      async: false,
      success: function(response)
      {
       window.server_public_key = response;
      },
  });
}
...
function getAndUsePubKeyAsync() {
  function asyncgetPubkey(){
     getPubkey()
     .done(function(response) {
       window.server_public_key  = response.replace(/\r/g, '');
     })
     .fail(function(x) {
       alert('Error: coldent get public key')
     });
  }
  $.ajax({
     url: './compute.php?renewKeyCheck=true',
     async: true,
     type: 'get',
     success: function(response){asyncgetPubkey()},
  });
}

//getAndUsePubKeyAsync()

//var intervalID = window.setInterval(getAndUsePubKey, 1000);

function checkIfServerWantsMakeNewKey(){
  $.ajax({
      url: './compute.php?renewKeyCheck=true',
      async: false,
      type: 'get',
...
function getPubkeyandMakeNewAsyncCallback(callback){
  function getPubkeyAsyncCallback(callback) {
    $.ajax({
      url: './compute.php',
      type: 'post',
      data: 'keyRequest=true',
  ...
  $.ajax({
      ...
      async: true,
      type: 'get',
      success: function(response){getPubkeyAsyncCallback(callback);}
   });
}
...
function compute(mode, inputtext, key){
 var computed = "";
 var operationSuccess = false;
 var operationGaveUp = false;
 var operationAttempts = 0;
 while(!operationGaveUp && !operationSuccess){
  ...
  var theForm = window.document.input;
  var input = inputtext;
  ...
  switch(mode){
	...
	var computed = pidCryptUtil.fragment(pidCryptUtil.encodeBase64(pidCryptUtil.convertFromHex(crypted)),64).replace(/\s+/g, '');
   ...
   case 'decrypt':
	...
	var computed = decrypted;
      }  else {/*alert('Could not find private key.');*/}
    break;
  }
  operationAttempts++;
  if(computed != ''){operationSuccess = true;}else{getPubkeyandMakeNew(); if(mode == 'encrypt'){key = window.server_public_key} if(mode == 'decrypt'){/*wtf thaths not soposed to happend*/}}
  if(operationAttempts >= 5){operationGaveUp = true;}
 }
 return computed;
}
...
// add '\n\' at end of line to have line breaks in string
var public_key_1024  = '-----BEGIN PUBLIC KEY-----\n\
MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQDXBnxD6A7d97mzrxvD8xNUWyur\n\
0HyecxJkEOCd4UOc16tbUb0KgRxgf9nsO+a8lC6uwYP+p2QZH8SFMYC72qzRDYxZ\n\
W5SfK+T8IkM/D9raUsbonL2KAPVzBFM0IZd/I8ZrMWeXdqPKCfheLOhMA6MI+KzG\n\
hh3n9v9Ww+3L0DXNMwIDAQAB\n\
-----END PUBLIC KEY-----';
**/
