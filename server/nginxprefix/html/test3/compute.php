...
<p>Upload file:<p>
<input class="upload-form" style="float: left;" type="file" name="upload_file" id="upload_file" multiple />
<button class="btn btn-primary" style="position: relative; bottom: 10px; left: 10px;" id="uploadfilebutton" type="button">Upload file</button>
<div id=\'progress\'></div>
 ...
 if($AuthenticationLevel != 'false'){
  ...
  if($AuthenticationLevel >= 1){
   ...
   if($_POST['Request'] == 'deletefile'){
    $file=str_replace('/', '', $_POST['filename']);
    if(file_exists('./data/' . $_POST['username'] . '/' . $file) && $file != ''){
     echo('<b>File Deleted</b>');
     if(is_dir('./data/' . $_POST['username'] . '/' . $file)){
      delTree('./data/' . $_POST['username'] . '/' . $file);
     }else{
      unlink('./data/' . $_POST['username'] . '/' . $file);
     }
    }else{
   ...
   if($_POST['Request'] == 'ListUserFiles'){
    ...
    $list .= '<tr><th valign="top"><img src="" alt="[ICO]"></th><th width="100%"><a href="">Name</a></th><th style="padding-left: 100px; white-space: nowrap;"><a href="">Last modified</a></th><th><a href="">Size</a></th><th><a href="">Actions</a></th><th><a href="">Status</a></th></tr>'."\n";
    ...
    foreach ($files as $k => $v){
     ...
     $type = '  ';
     if(filetype($path) == 'dir') $type = 'DIR';
     $list .= '<tr><td valign="top"><img src="" alt="[' . $type . ']"></td>' . '<td><a class="unselectable" onclick="AuthGetFile('."'".str_replace("'", "!ApostropheChar!", $path)."'".')">' . $name .'</a></td>' . '<td align="right">' . $datemodified . '</td>' . '<td align="right">' . $size . '</td><td><a class="unselectable" onclick="deleteuserfile('."'".str_replace("'", "!ApostropheChar!", $v)."'".')">DEL</a>&nbsp;</td><td><div id="userfilesstatusdisplay:'.$v.'"></div></td></tr>' . "\n";
    }
    $list .= '<tr><th colspan="6"><hr class="hr123"></th></tr>'."\n";
    $list .= "</table>\n</body>\n</html>";
   ...
   if($_POST['Request'] == 'RequestValidateUpload'){
    if(isset($_POST['UploadKey'])){
     include_once("upload.php");
     $uploadDatabase=new uploadDatabase();
     $uploadDatabase->CleanOldUploads();
     $uploadDatabase->ValidateUpload($_POST['UploadKey'], "./data/".str_replace('/', '', $_POST['username']).'/');
    }
   }
  }
 }else{echo 'Wrong key';}
