...
*/
	...
	$config = array(
		'mpd_host' => '192.168.1.143',
		'mpd_port' => '6600',
		'mpd_pass' => 'MPD_SERVER#12jh3v123fg',
		'update_delay' => '1',
		'theme' => 'default',
		'lang' => 'en',
		'Title' => 'yes',
	...
	if(isset($_GET['playlist'])) {
	...
	else if(isset($_GET['currentsongCC'])) {
		$res = "failure";
		try {
			$res = $pl->getCurrentSong();
			if(!$res) 
	...
	else if(isset($_GET['listsongsCC'])) {
		$res = "failure";
		try {
			$res = $pl->getPlaylistInfoId();
			if(!$res) 
				$res = "failure";
		}
		catch(PEAR_Exception $e) {
		...
		//var_dump($res);
		$list="";
		foreach($res['file'] as $song)
		{
		 $list .= $song['file'].':'.$song['Id'].",";
		}
